...
import { Employee, Shift, AppView, WeeklyStats, AvailabilityRequest, Notification } from '@/src/types';
import { INITIAL_EMPLOYEES, INITIAL_SHIFTS, INITIAL_WEEKLY_BUDGET, INITIAL_REQUESTS, INITIAL_NOTIFICATIONS, formatTime } from '@/src/constants';
import { StatsCard } from '@/src/components/StatsCard';
import { checkVersionSync, logVersionInfo } from '@/src/lib/versionCheck';
import { useAuth } from '@/src/contexts/AuthContext';
import { signOut } from '@/src/lib/firebase/auth';

// Lazy load heavy components for better performance
const ScheduleGrid = lazy(() => import('@/src/components/ScheduleGrid').then(m => ({ default: m.ScheduleGrid })));
const CompliancePanel = lazy(() => import('@/src/components/CompliancePanel').then(m => ({ default: m.CompliancePanel })));
const TeamView = lazy(() => import('@/src/components/TeamView').then(m => ({ default: m.TeamView })));
const EmployeePortal = lazy(() => import('@/src/components/EmployeePortal').then(m => ({ default: m.EmployeePortal })));
